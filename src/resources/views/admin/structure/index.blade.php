...
@include('partials.breadcumb', ['title' => 'Struktur Jabatan'])
<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="py-6 px-4 md:px-6 xl:px-7.5 flex justify-end items-center">
		@if(in_array(App\Constant\Permission::CAN_CREATE_STRUCTURE, $permissions))  	
			<a href="{{ route('structure.create.form') }}" class="flex items-center justify-center rounded-md bg-primary py-2 px-10 text-white hover:bg-opacity-95">Buat baru</a>
		@endif
    </div>
    ...
    <div
		...
		@if(in_array(App\Constant\Permission::CAN_UPDATE_STRUCTURE, $permissions) || in_array(App\Constant\Permission::CAN_DELETE_STRUCTURE, $permissions))
    ...
    <div class="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
        <div class="col-span-3 flex items-center">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-center">
                <img src="{{ $item->avatar }}" alt="Product" class="h-15 w-15 rounded-full" />
                <p class="font-medium text-sm text-black dark:text-white">{{ $item['name']; }}</p>
            </div>
        </div>
        <div class="col-span-3 flex items-center">
            <button class="inline-flex rounded-full bg-[#13C296] py-1 px-3 text-sm font-extralight text-white hover:bg-opacity-90">
                {{ $item['title']; }}
            </button>
        </div>
        <div class="flex items-center space-x-3.5">
			@if(in_array(App\Constant\Permission::CAN_DELETE_STRUCTURE, $permissions))
            <a href="{{ route('structure.delete', ['structureId' => $item['id']]) }}">
			...
			@endif
			@if(in_array(App\Constant\Permission::CAN_UPDATE_STRUCTURE, $permissions))
            <a
                href="{{ route('structure.update.form', ['structureId' => $item['id']]) }}"
