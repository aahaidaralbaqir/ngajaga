...
<main x-data="activity"
	  x-init="
	  	image_icon_url = '@php echo empty($item) ? '' : $item->payment_logo @endphp';
    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <!-- ====== Settings Section Start -->
            <div
                ...
                <div class="p-7">
				@php
					$form = Form::open(['route' => 'payment.update', 'method' => 'POST', 'enctype' => 'multipart/form-data']);
					if (empty($item)) $form = Form::open(['route' => 'payment.create', 'method' => 'POST', 'enctype' => 'multipart/form-data']);	
				@endphp 
                {{ $form }}
                    <div class="mb-5.5">
                        ...
                        <div class="mb-5.5">
                            ...
                            <div class="relative z-20 bg-transparent dark:bg-form-input">
                                <select name="id_parent" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                                    <option value="0">Pilih Payment</option>
                                    @foreach ($payments as $payment)
                                        @php
                                            $selected = FALSE;
                                            if (!empty($item) && $item->id_parent == $payment->id)
                                                $selected = TRUE;
                                        @endphp 
                    ...
                    <div class="mb-5.5 flex gap-2">
                        <div class="w-1/2">
                            ...
                            <input type="text" name="value"
                                value="{{ old('value', !empty($item->value) ? $item->value : '') }}"
                                class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                            @error('value')
                                <span class="text-sm text-danger">{{ $message }}</span>
                            @enderror
                        ...
                        <div class="w-1/2">
                            ...
                            <input type="number" name="expired_time"
                                value="{{ old('expired_time', !empty($item->expired_time) ? $item->expired_time : '') }}"
                                class="w-full rounded-lg border-[1.5px] border-stroke bg-transparent py-3 px-5 font-medium outline-none transition focus:border-primary active:border-primary disabled:cursor-default disabled:bg-whiter dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary" />
                            @error('value')
                                <span class="text-sm text-danger">{{ $message }}</span>
                            @enderror
