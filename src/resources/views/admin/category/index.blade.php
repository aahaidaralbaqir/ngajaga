    ...
    <div class="w-4/5">
        <div class="flex justify-between items-center">
            ...
            <div class="flex items-center justify-between gap-5 relative">
                ...
                @if (in_array(\App\Constant\Permission::CREATE_CATEGORY, $user['permission']))
                    <a href="{{ route('category.create.form') }}" class="button text-base bg-[#ff91e7] text-black p-3 rounded border border-black">Buat Kategori</a>
                @endif
            </div>
        </div>
        <div class="tab">
            ...
            @if (in_array(\App\Constant\Permission::VIEW_CATEGORY, $user['permission']))
                <a href="{{ route('category.index') }}" aria-selected="true" class="selected">Kategori</a>
            @endif
...
<main class="w-full py-14 px-14">
    <section class="w-4/5">
        <table class="w-full retro">
            <caption class="text-2xl text-black [text-align:unset]">Kategori</caption>
            <thead>
                <tr>
                    <th class="w-[5%]"></th>
            ...
            <tbody>
                @foreach($category as $item)
                    <tr>
                        ...
                        <td class="relative">
                            @if (in_array(\App\Constant\Permission::UPDATE_CATEGORY, $user['permission']))
                            ...
                            <div class="menu hidden" data-id="{{ $item->id }}" data-name="action" role="dropdown-content">
                                <a href="{{ route('category.edit.form', ['categoryId' => $item->id]) }}" class="menu-item">
