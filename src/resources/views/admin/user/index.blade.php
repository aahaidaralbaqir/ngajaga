...
@include('partials.breadcumb', ['title' => 'User'])
<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="py-6 px-4 md:px-6 xl:px-7.5 flex justify-end items-center">
    	<a href="{{ route('user.create.form') }}" class="flex items-center justify-center rounded-md bg-primary py-2 px-10 text-white hover:bg-opacity-95">Buat baru</a>
    </div>
    ...
    <div class="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
        ...
        <div class="col-span-2 flex items-center">
            <p class="font-medium text-sm text-black dark:text-white">{{ $item['email']; }}</p> 
        </div>
        <div class="col-span-2 flex items-center">
            <button 
                class="inline-flex rounded-full bg-[#13C296] py-1 px-3 text-sm font-extralight text-white hover:bg-opacity-90"
            >
                {{ $item->roles->name }}
            </button>
        </div>
        <div class="flex items-center space-x-3.5">
            <a href="{{ route('user.delete', ['userId' => $item['id']]) }}">
            ...
            <a
                href="{{ route('user.update.form', ['userId' => $item['id']]) }}"
