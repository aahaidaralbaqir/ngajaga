...
@include('partials.breadcumb', ['title' => 'Penerima Dana'])
<div class="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
    <div class="py-6 px-4 md:px-6 xl:px-7.5 flex justify-end items-center">
    	<a href="{{ route('beneficiary.create.form') }}" class="flex items-center justify-center rounded-md bg-primary py-2 px-10 text-white hover:bg-opacity-95">Buat penerima dana</a>
    </div>
    ...
    <div
        ...
        <div class="col-span-2 flex items-center">
            <p class="font-medium">Kategoryi</p>
        </div>
		@if(in_array(App\Constant\Permission::CAN_UPDATE_BENEFICIARY, $permissions))
    ...
    <div class="grid grid-cols-6 border-t border-stroke py-4.5 px-4 dark:border-strokedark sm:grid-cols-8 md:px-6 2xl:px-7.5">
        ...
        <div class="flex items-center space-x-3.5">
			...
			@endif
			@if(in_array(App\Constant\Permission::CAN_UPDATE_BENEFICIARY, $permissions))
            <a href="{{ route('beneficiary.update.form', ['beneficiaryId' => $item->id]) }}" class="hover:text-primary">
                <svg class="fill-current" width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
