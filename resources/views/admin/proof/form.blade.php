...
<main x-data="activity"
	  x-init="
	  	image_banner_url = '@php echo empty($item) ? '' : $item->image @endphp';
	  "
	>
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <!-- ====== Settings Section Start -->
            <div
                ...
                <div class="p-7">
				@php
					$form = Form::open(['route' => 'transaction.proof', 'method' => 'POST', 'enctype' => 'multipart/form-data']);
				@endphp 
                {{ $form }}
                    <div class="mb-5.5 lg:flex lg:gap-4.5">
						<input type="hidden" name="transaction_id" value="{{ $transaction_record->id }}">
                        <div class="flex-1">
                    ...
                    <div class="flex justify-end gap-4.5">
						@if ($transaction_record->transaction_status == \App\Constant\Constant::TRANSACTION_REQUESTED && in_array(\App\Constant\Permission::CAN_APPROVE_TRANSACTION, $permissions))
							<a href="{{ route('transaction.admin.approve', ['transactionId' => $transaction_record->order_id]) }}" class="flex justify-center rounded bg-[#212B36] py-2 px-6 font-medium text-gray hover:bg-opacity-90">
								Setujui
							</a>
						@endif
						<button
