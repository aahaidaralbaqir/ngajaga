    ...
    <div class="mx-auto max-w-screen-2xl p-4 md:p-6 2xl:p-10">
        ...
        <!-- ====== Settings Section Start -->
            <div
                ...
                <div class="p-7">
				@php
					$form = Form::open(['route' => 'structure.update', 'method' => 'POST', 'enctype' => 'multipart/form-data']);
					if (empty($item)) $form = Form::open(['route' => 'structure.create', 'method' => 'POST', 'enctype' => 'multipart/form-data']);	
				@endphp 
                {{ $form }}
					<div class="mb-5.5">
						...
						<div class="relative z-20 bg-transparent dark:bg-form-input">
							<select name="id_transaction_type" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
								...
								@foreach ($transaction_type as $item)
									@php
										$selected = FALSE;
										if (!empty($item) && $item->id_transaction_type == $item->id)
											$selected = TRUE;
									@endphp 
					...
					<div class="mb-5.5">
						...
						<div class="relative z-20 bg-transparent dark:bg-form-input">
							<select name="id_transaction_tpe" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
								<option value="0">Pilih Satuan</option>
								@foreach ($unit as $item)
									@php
										$selected = FALSE;
										if (!empty($item) && $item->unit_id == $item->id)
											$selected = TRUE;
									@endphp 
									<option {{ $selected ? 'selected' : '' }} value="{{ $item->id }}">{{  $item->name }}</option>
								@endforeach
