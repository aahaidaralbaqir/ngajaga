...
<main x-data="activity"
	  x-init="
	  	image_banner_url = '@php echo empty($item) ? '' : $item->icon @endphp' 
	  "
	>
        ...
        <!-- ====== Settings Section Start -->
            <div
                ...
                <h3 class="font-medium text-black dark:text-white">
					@if (empty($item))
						Menambahkan jenis transaksi baru
					@else
						Mengupdate jenis transaksi baru 
					@endif
                </h3>
                </div> 
                <div class="p-7">
				@php
					$form = Form::open(['route' => 'transaction.type.update', 'method' => 'POST', 'enctype' => 'multipart/form-data']);
					if (empty($item)) $form = Form::open(['route' => 'transaction.type.create', 'method' => 'POST', 'enctype' => 'multipart/form-data']);	
				@endphp 
                {{ $form }}
                    <div class="mb-5.5">
						...
						@endif
                        <input type="text" placeholder="Isi nama jenis transaksi" name="name"
					...
					<div class="mb-5.5">
						...
						<select name="status" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
							...
							@foreach ($status as $key => $value)
								@php
									$selected = FALSE;
									if (!empty($item) && $item->status == $value)
										$selected = TRUE;
								@endphp 
                    ...
                    <div class="mb-5.5">
                        ...
                        <textarea rows="6" placeholder="Isi deskripsi jenis transaksi" name="description"
