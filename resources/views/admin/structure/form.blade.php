...
<main x-data="activity"
	  x-init="
	  	image_icon_url = '@php echo empty($item) ? '' : $item->avatar @endphp';
	  "
	>
            ...
            <div
                ...
                {{ $form }}
                    <div class="mb-5.5">
						...
						@endif
                        <input type="text" placeholder="Nama orang" name="name"
                            ...
                            <div class="relative z-20 bg-transparent dark:bg-form-input">
                                <select name="id_parent" class="relative z-20 w-full appearance-none rounded border border-stroke bg-transparent py-3 px-5 outline-none transition focus:border-primary active:border-primary dark:border-form-strokedark dark:bg-form-input dark:focus:border-primary">
                                    ...
                                    @foreach ($structures as $structure)
                                        @php
                                            $selected = FALSE;
                                            if (!empty($item) && $item->id_parent == $structure->id)
                                                $selected = TRUE;
                                        @endphp 
                                        <option {{ $selected ? 'selected' : '' }} value="{{ $structure->id }}">{{  $structure->title . ' - ' . $structure->name }}</option>
                                    @endforeach
                                </select>
